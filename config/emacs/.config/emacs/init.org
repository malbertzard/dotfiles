...
#+begin_src emacs-lisp
    (defvar my/managed-buffer-last-buffer nil)

    (defun my/tab-name ()
      (alist-get 'name (tab-bar--current-tab)))

    (defun my/managed-buffer-name (kind)
      (format "*%s:%s*" kind (my/tab-name)))

    (defmacro my/define-tab-buffer (name create-fn key)
      (let ((fn (intern (format "my/tab-%s-toggle" name))))
        `(progn
           (defun ,fn ()
             (interactive)
             (let* ((buf-name (my/managed-buffer-name ,name))
                    (buf (get-buffer buf-name)))

               (if (and buf (eq (current-buffer) buf))
                   (when my/managed-buffer-last-buffer
                     (pop-to-buffer my/managed-buffer-last-buffer))

                 (setq my/managed-buffer-last-buffer (current-buffer))

                 (if buf
                     (pop-to-buffer buf)
                   (let ((new (funcall ,create-fn)))
...
#+begin_src emacs-lisp :tangle no
    (leaf ghostel
      :ensure t
      :bind
      (("C-c t T" . ghostel-with-prefix)
       ("C-c t t" . my/tab-ghostel-toggle))
      :config

      (defun ghostel-with-prefix ()
    	(interactive)
      ...
      (defun ghostel-other-window-with-prefix ()
    	(interactive)
    	(other-window 1)
    	(let ((current-prefix-arg '(4)))
    	  (call-interactively #'ghostel)))

      (my/define-tab-buffer
       "ghostel"
       #'ghostel-other-window
       "C-c t t"))
#+end_src
...
#+begin_src emacs-lisp
    (leaf vterm
      ...
      :bind
      (("C-c t T" . vterm-with-prefix)
       ("C-c t t" . my/tab-vterm-toggle))
      :config
...
#+begin_src emacs-lisp
    (leaf compile
      ...
      (("C-c C c" . my/compile)
       ...
       ("C-c C e j" . compilation-next-error)
       ("C-c C e k" . compilation-previous-error)
       ("C-c C e l" . consult-compile-error))

      :config

      ;; tab-local compile entry point
      (defun my/compile ()
        (interactive)
        (let ((compilation-buffer-name-function
               (lambda (_)
                 (my/managed-buffer-name "compilation"))))
          (call-interactively #'compile)))

      ;; toggle behavior via macro
      (my/define-tab-buffer
       "compilation"
       (lambda ()
         (my/compile)
