...
*** Terminal
#+BEGIN_SRC emacs-lisp
    (defun eat-with-prefix ()
      "Run `eat` with a universal- argument (C-u)."
      (interactive)
      (let ((current-prefix-arg '(4))) ; C-u
        (call-interactively 'eat)))

    (defun eat-other-window-with-prefix ()
      "Run `eat` with a universal- argument (C-u)."
      (interactive)
      (let ((current-prefix-arg '(4))) ; C-u
        (call-interactively 'eat-other-window)))

    (global-set-key (kbd "C-c t T") 'eat-with-prefix)
    (global-set-key (kbd "C-c t t") 'eat-other-window-with-prefix)
#+END_SRC
