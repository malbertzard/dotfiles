...
- [[#performace][Performace]]
  - [[#compile-external-packages][Compile External Packages]]
  - [[#better-gc][Better GC]]
  - [[#view-large-filesvfl][View large files(VFL)]]
...
- [[#completion-systems][Completion Systems]]
  ...
  - [[#useful-commands-consult][Useful commands Consult]]
  - [[#templates][Templates]]
...
- [[#helper-packages][Helper packages]]
  - [[#midnight-commander--cleanup][Midnight commander / cleanup]]
...
* External Packages
** Use Package
Depending on if we use nixos or not use package or not
#+begin_src emacs-lisp
    ...
    (setq use-package-always-ensure (not my/use-nix-packages))
#+end_src
** Keep folders clean
#+begin_src emacs-lisp
    (unless my/use-nix-packages
...
* Normal Defaults
** Sensible Defaults
Hide unused UI Elements for cleaner look
#+BEGIN_SRC emacs-lisp
    (use-package emacs
      :ensure nil
      :custom
...
We do not want trash files everywhere
#+BEGIN_SRC emacs-lisp
    (use-package emacs
      :ensure nil
      :custom
...
#+BEGIN_SRC emacs-lisp
    (use-package tempel
      ...
      (tempel-path
       (list (expand-file-name "tempel/*" user-emacs-directory)))
      :init
...
#+begin_src emacs-lisp
    (setq window-sides-slots '(1 2 3 4))
    ...
    (setq display-buffer-alist
    	  '(
    		...
    		;; -----------------------------
    		;; Diff / ediff in frames (unchanged behavior)
    		;; -----------------------------
    ...
    (use-package meow
      ...
      (defun meow-setup ()
    	...
    	(meow-normal-define-key
    	 ...
    	 '("Z" . avy-goto-char) ;; TODO
    	 (cons "S" surround-keymap) ;; TODO
...
#+begin_src emacs-lisp
    (defvar my-local-config
      (expand-file-name "~/.config/emacs/local.el")
      "Local configuration file for this machine.")

    (when (file-exists-p my-local-config)
      (load my-local-config))
#+END_SRC
