...
** Shackle
#+begin_src emacs-lisp
    ;; Ensure shackle is installed
    ;; Ensure shackle is installed
    (use-package shackle
      :ensure t
      :init
      ;; Enable shackle globally
      (shackle-mode 1)

      (setq shackle-default-rule '(:select nil))  ;; Focus popups by default

      (setq shackle-rules
            '(
              ;; File-related stuff (dired, find-file, grep) on the left
              ("*dired*" :align left :size 0.25 :select t)
    		  ("*dired*" :mode dired-mode :align left :size 0.25 :select t)
              ("*grep*"  :align left :size 0.25)
              ("*find-file*" :align left :size 0.25 :select t)

              ;; Help/Info messages go to the right
              ("*Help*" :align right :size 0.30)
              ("*Messages*" :align right :size 0.20)
              ("*Apropos*" :align right :size 0.25)
              ("*info*" :align right :size 0.35)
    		  
              ("*Diff*" :frame t)

              ;; Compilation buffers go to the bottom
              ("*compilation*" :align below :size 0.30)
              ("*shell*" :align below :size 0.30 :select t)
              ("*vterm*" :align below :size 0.30 :select t)
              ("*ansi-term*" :align below :size 0.30 :select t)

              ;; Magit, Org-mode-related buffers at bottom too
              ("*Magit*" :align below :size 0.40)
              ("*Org Agenda*" :align below :size 0.35)

              ;; General fallback for any other popups
              ("\\*.*\\*" :regexp t :align below :size 0.30)
    		  
              ("*.*" :regexp t :align t :select t :inhibit-window-quit t)
              )))
#+end_src
* Editing
** Jump to char
...
#+begin_src emacs-lisp
    (use-package vterm
      ...
      (global-set-key (kbd "C-c t t") 'vterm-other-window-with-prefix)
      :ensure t)
#+END_SRC
...
#+begin_src elisp
    ...
    (defun scad-preview ()
      "Render current OpenSCAD file to PNG and display."
      (interactive)
      (let ((png (concat (file-name-sans-extension buffer-file-name) ".png")))
        (compile
         (format "openscad --imgsize=800,600 -o %s %s"
                 png buffer-file-name))
        (find-file-other-window png)))
