...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package org-roam
      ...
      (org-roam-capture-templates
       '(("p" "Project" plain "%?" :if-new (file+head "projects/${slug}.org" "#+title: ${title}\n"))
         ("a" "Area" plain "%?" :if-new (file+head "areas/${slug}.org" "#+title: ${title}\n"))
         ("r" "Resource" plain "%?" :if-new (file+head "resources/${slug}.org" "#+title: ${title}\n"))
         ("P" "Archive" plain "%?" :if-new (file+head "archives/${slug}.org" "#+title: ${title}\n")))))

    (defun my/org-roam-index ()
      "Generate an index of all notes in the org-roam directory with tags."
      (interactive)
      (let ((index-file (expand-file-name "index.org" org-roam-directory)))
        (with-temp-buffer
          (insert "#+TITLE: Org Roam Index\n\n")
          (insert "** Projects\n")
          (dolist (file (directory-files (expand-file-name "projects" org-roam-directory) t "\\.org$"))
            (insert (format "- [[file:projects/%s][%s]]\n" (file-name-nondirectory file) (file-name-base file))))
          (insert "\n** Areas\n")
          (dolist (file (directory-files (expand-file-name "areas" org-roam-directory) t "\\.org$"))
            (insert (format "- [[file:areas/%s][%s]]\n" (file-name-nondirectory file) (file-name-base file))))
          (insert "\n** Resources\n")
          (dolist (file (directory-files (expand-file-name "resources" org-roam-directory) t "\\.org$"))
            (insert (format "- [[file:resources/%s][%s]]\n" (file-name-nondirectory file) (file-name-base file))))
          (insert "\n** Archives\n")
          (dolist (file (directory-files (expand-file-name "archives" org-roam-directory) t "\\.org$"))
            (insert (format "- [[file:archives/%s][%s]]\n" (file-name-nondirectory file) (file-name-base file))))
          (write-file index-file)
          (message "Org-roam index file updated."))))
#+END_SRC
